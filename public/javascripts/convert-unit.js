                    ...
                    if (rel.length){
                        rel.map(function (e) {
                            $($(dropdownElement).children()[0]).append('<li class="dropdown-item">\n' +
                                ...
                                '                                                    <div class="text-name-option current-code" ><span style="font-size: 1.5em;font-weight: 600;line-height: 20%">'+e.TenDonVi+'</span></div>\n' +
                                '                                                    <div class="text-name-option current-name"><span>'+e.KiHieu+'</span></div>\n' +
                                '                                                </div>\n' +
                                '                                            </a>\n' +
                                '                                        </li>')
                        })
...
function displaySelectUnitToInput(Dropdown,currentUnitSelect,dropdownElement,searchInput) {
    $(Dropdown).children().bind('click',function () {
        ...
        let nameUnit = $($(unitSelect).children()[0]).text();
        $($($(currentUnitSelect).children()[1]).children()[0]).text(symbolUnit);//set symbol div tag to input box
        $($($(currentUnitSelect).children()[0]).children()[0]).text(nameUnit);// set name div tag to input box
...
function clickInverseButton() {
    ...
    $(btnInverse).bind('click',function () {
        ...
        let nameUnitInput1 = $($($(selectInput1).children()[0]).children()[0]).text();
        let symbolInput2 = $($($(selectInput2).children()[1]).children()[0]).text();
        let nameUnitInput2 = $($($(selectInput2).children()[0]).children()[0]).text();
        ...
        $($($(selectInput2).children()[0]).children()[0]).text(nameUnitInput1);
        $($($(selectInput1).children()[1]).children()[0]).text(symbolInput2);
        $($($(selectInput1).children()[0]).children()[0]).text(nameUnitInput2);
    })
}
clickInverseButton();
